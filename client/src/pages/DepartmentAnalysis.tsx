...
export default function DepartmentAnalysis() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500" data-testid="department-analysis-page">
      <header className="flex flex-col md:flex-row justify-between md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-foreground" data-testid="text-page-title">Department Analysis</h1>
          <p className="text-muted-foreground mt-1">Department-wide performance and satisfaction metrics</p>
        </div>
      ...
      <section data-testid="section-sentiment-distribution">
        <Card>
          <CardHeader>
            <CardTitle>Sentiment Distribution</CardTitle>
            <CardDescription>Employee wellness breakdown by department ({period}) -- how many employees fall into each sentiment tier</CardDescription>
          </CardHeader>
      ...
      <section data-testid="section-top-blockers">
        <Card>
          <CardHeader>
                ...
                {Object.entries(blockersByDept).map(([dept, items]) => (
                  <div key={dept}>
                    ...
                    <div className="space-y-2 pl-6">
                      ...
                      {items.length > 5 && (
                        <p className="text-xs text-muted-foreground pl-1">+{items.length - 5} more blockers in {dept}</p>
                      )}
                    </div>
                  </div>
                ))}
